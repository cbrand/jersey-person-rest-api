...
public class PersonSearcher extends Searcher<Person> {
	...
	{
		this.updateWith(
				having(on(Person.class).getLastName(), equalTo(last_name))
				);
	}
	...
	{
		this.updateWith(
			having(on(Person.class).getFirstName(), equalTo(first_name))
		);
	}
	...
	{
		this.updateWith(
			having(on(Person.class).getEmailAddress(), equalTo(email_address))
		);
	}
	...
	{
		this.updateWith(
			having(on(Person.class).getLocation().getLatitude(), equalTo(latitude))
		);
		this.updateWith(
			having(on(Person.class).getLocation().getLongitude(), equalTo(longitude))	
		);
	}
	...
	{
		this.updateWith(
			having(on(Person.class).getBirthDate(), equalTo(birth_date))	
		);
	}
